...
_In the appendix a random sample snapshot of the data can be found to provide a tangible sense of the data structure._
...
ggplot(data = combined_df, aes(x = Date, y = value)) +
  ...
  geom_point(color = "#E74C3C", size = 0.2, alpha = 0.7) +  
...
For the ARIMA with Dynamic Regression model, we took the best ARIMA model for each of the cantons and added exogenous variables, a dummy variable for **Covid** was added to both, and the **exchange rate** between the Japanese Yen and Swiss Franc (JPY/CHF) was also added to Luzern's ARIMA.

## TSLM

For TSLM a model was created for Luzern that included the exchange rate between the Japanese Yen and Swiss Franc (JPY/CHF) and a Covid dummy. The model was created using the following formula:
